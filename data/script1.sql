...
VALUES
    ...
    ('FleetMind', 'Seed', 'AI route optimizer for logistics fleets', 'Route optimization software that accounts for real-time traffic, weather, and vehicle constraints to maximize delivery efficiency. Features include dynamic rerouting, driver mobile app integration, and predictive maintenance alerts.', '$350K', 'Ex-Uber Maps, DHL', '$5.9B', '24% YoY', 'Early market entry', '15% MoM', '25% QoQ', '$20M', '$220M', '4-5 years'),
    
    ('Syncr', 'Pre-Seed', 'AI memory layer for knowledge workers', 'Browser extension that captures and organizes information encountered during online research. Uses AI to tag, connect, and resurface relevant information when needed, creating a personalized knowledge graph.', '$20K', 'YC batch, ex-Notion', '$4.3B', '23% YoY', 'Private beta', '60% MoM (small base)', '40% MoM', '$6M', '$85M', '6-7 years'),
    
    ('TruAudit', 'Series A', 'Compliance automation tool for SaaS startups', 'Platform that streamlines SOC2, GDPR, and other compliance processes through continuous monitoring and automated evidence collection. Features include policy templates, vendor risk assessment, and audit-ready reporting.', '$870K', 'Ex-SOC2 consultants + engineer', '$3.8B', '19% YoY', 'Revenue scaling', '12% QoQ', '22% QoQ', '$32M', '$240M', '3-5 years'),
    
    ('MetaBriefs', 'Seed', 'AI briefings for busy execs using internal company data', 'AI system that aggregates internal company data (emails, documents, dashboards) to create personalized daily briefings for executives. Features include custom alert thresholds, competitive intelligence integration, and voice-enabled interfaces.', '$300K', 'Ex-McKinsey, ex-Palantir', '$7.2B', '25% YoY', 'Product-market fit', '18% MoM', '30% QoQ', '$18M', '$210M', '4-6 years');

-- Sample queries to verify data

-- Query 1: Get all pre-seed companies
-- SELECT * FROM startups WHERE stage = 'Pre-Seed';

-- Query 2: Get companies with ARR over $1M
-- SELECT company_name, arr, current_valuation FROM startups WHERE arr LIKE '$%M';

-- Query 3: Get companies sorted by current valuation (highest to lowest)
-- SELECT company_name, current_valuation, projected_exit_value FROM startups ORDER BY CAST(REPLACE(REPLACE(current_valuation, '$', ''), 'M', '') AS REAL) DESC;

-- Query 4: Get average TAM by funding stage
-- SELECT stage, AVG(CAST(REPLACE(REPLACE(tam, '$', ''), 'B', '') AS REAL)) AS avg_tam_billions FROM startups GROUP BY stage;

-- Query 5: Get companies with AI in their product description
-- SELECT company_name, product_short FROM startups WHERE description LIKE '%AI%';